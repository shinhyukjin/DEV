...
class Trainer(object):
    def __init__(self,
        ...
        self.teadcher_model = torch.nn.DataParallel(teacher_model).to(self.device)
    ...
    def compute_e0_loss(self):
        ...
        with torch.no_grad():        
            for batch_idx, (inputs,calibs,coord_ranges, targets, info) in enumerate(self.train_loader):
                ...
                outputs = self.model(inputs,coord_ranges,calibs,targets,teacher_feat)
                ...
                progress_bar.update()
                if 1:
                    print("test lib helper trainhelper 120line")
                    return disp_dict
