...
def load_teacher_head_checkpoint(model, optimizer, filename, logger, map_location):
    if os.path.isfile(filename):
        logger.info("==> Loading student from teacher head checkpoint '{}'".format(filename))
        checkpoint = torch.load(filename, map_location=map_location)
        epoch = checkpoint.get('epoch', -1)
        if model is not None and checkpoint['model_state'] is not None:
...
def load_teacher_checkpoint(teacher_model, filename, logger, map_location):
    if os.path.isfile(filename):
        logger.info("==> Loading from teacher checkpoint '{}'".format(filename))
        checkpoint = torch.load(filename, map_location=map_location)
        if teacher_model is not None and checkpoint['model_state'] is not None:
