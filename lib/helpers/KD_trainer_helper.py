...
class KD_Trainer(object):
    def __init__(self,
        ...
        self.cfg_teacher = cfg['teacher_model']
        self.cfg_student = cfg['model']
        self.model = model
        ...
        load_teacher_checkpoint(self.teacher_model, self.cfg_teacher['resume_model'], self.logger,
                                     map_location=self.device)

        self.model = torch.nn.DataParallel(model).to(self.device)
        #self.model = torch.nn.DataParallel(model).to(device=torch.device("cuda"))

        self.teacher_model = torch.nn.DataParallel(teacher_model).to(self.device)
        #self.teadcher_model = torch.nn.DataParallel(teacher_model).to(device=torch.device("cuda"))
    ...
    def compute_e0_loss(self):
        ...
        with torch.no_grad():        
            for batch_idx, (inputs,calibs,coord_ranges, targets, info) in enumerate(self.train_loader):
                ...
                if self.debug == 1:
                    print("warning! debugging mode: lib/helper/KD_train_helper - 145line")
                    return disp_dict
