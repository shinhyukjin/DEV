...
class GUPNet(nn.Module):
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, K=50, mode='train', stu_ret = None):
        ...
        if mode == 'teacher':
            ret['heatmap'] = self.heatmap(feat)
            ret['offset_2d'] = stu_ret['offset_2d']
            ret['size_2d'] = stu_ret['size_2d']
            ret['feat'] = feat
        else :
            ret['heatmap']=self.heatmap(feat)
            ret['offset_2d']=self.offset_2d(feat)
        ...
        #two stage
        assert(mode in ['train','val','test','teacher'])
    ...
    def get_roi_feat_by_mask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            if 0:  ## 13x13
                # get roi feature
                roi_feature_masked = roi_align(feat, box2d_masked, [13, 13])
                # get coord range of each roi
                coord_ranges_mask2d = coord_ranges[box2d_masked[:, 0].long()]
                ...
                coord_maps = torch.cat([torch.cat([coords_in_camera_coord[:, 1:2] + i * (
                            coords_in_camera_coord[:, 3:4] - coords_in_camera_coord[:, 1:2]) / 12 for i in range(13)],
                                                  -1).unsqueeze(1).repeat([1, 13, 1]).unsqueeze(1),
                                        torch.cat([coords_in_camera_coord[:, 2:3] + i * (
                                                    coords_in_camera_coord[:, 4:5] - coords_in_camera_coord[:, 2:3]) / 12
                                                   for i in range(13)], -1).unsqueeze(2).repeat([1, 1, 13]).unsqueeze(1)],
                                       1)
                ...
                roi_feature_masked = torch.cat(
                    [roi_feature_masked, coord_maps, cls_hots.unsqueeze(-1).unsqueeze(-1).repeat([1, 1, 13, 13])], 1)
